...
export function buildCampaignReport(_rows, _dateFrom, _dateTo) {
  ...
  for (const row of dailyRows) {
    if (exact && !byCampaign.has(row.campaignId)) continue;
    const current = byCampaign.get(row.campaignId) ?? { period: [], daily: [] };
...
export function selectCampaignSource(source, apiArticles, xlsxArticles) {
  return source === "xlsx" ? xlsxArticles : apiArticles;
}

export function compareCampaignSources(apiArticles, xlsxArticles) {
  const keys = ["views", "clicks", "expense", "directOrders", "modelOrders", "directSales", "modelSales"];
  const empty = () => Object.fromEntries(keys.map(key => [key, 0]));
  const add = (sum, cell) => { for (const key of keys) sum[key] += Number(cell[key] || 0); return sum; };
  const collect = articles => {
    const map = new Map();
    for (const article of articles) for (const campaign of article.campaigns) {
      const current = map.get(String(campaign.id)) ?? { name: campaign.name, ...empty() };
      const cells = campaign.periodTotals ? [campaign.periodTotals] : Object.values(campaign.dates ?? {});
      cells.forEach(cell => add(current, cell));
      map.set(String(campaign.id), current);
    }
    return map;
  };
  const api = collect(apiArticles), xlsx = collect(xlsxArticles);
  const rows = [...new Set([...api.keys(), ...xlsx.keys()])].map(id => ({
    id, name: xlsx.get(id)?.name ?? api.get(id)?.name ?? id,
    api: api.get(id) ?? null, xlsx: xlsx.get(id) ?? null,
    expenseDelta: (xlsx.get(id)?.expense ?? 0) - (api.get(id)?.expense ?? 0),
  })).sort((a, b) => Math.abs(b.expenseDelta) - Math.abs(a.expenseDelta));
  return { rows, apiTotals: [...api.values()].reduce(add, empty()), xlsxTotals: [...xlsx.values()].reduce(add, empty()), apiCount: api.size, xlsxCount: xlsx.size };
}

export async function collectCampaignPages(fetchPage) {
  const campaigns = new Map();
  for (let page = 1; page <= 100; page++) {
    const payload = await fetchPage(page);
    const before = campaigns.size;
    for (const campaign of payload.list ?? []) campaigns.set(String(campaign.id ?? campaign.campaignId), campaign);
    const total = Number(payload.total);
    if (Number.isFinite(total) && campaigns.size >= total) return [...campaigns.values()];
    if (!payload.list?.length && !Number.isFinite(total)) return [...campaigns.values()];
    if (campaigns.size === before) throw new Error("Ozon вернул неполный список кампаний. Обновите данные позже.");
  }
  throw new Error("Не удалось загрузить все страницы кампаний Ozon");
}
