...
export default function Home() {
  ...
  const updatePricingCompetitor = useCallback(async (row: TargetPriceRow, competitorNmId: number, action: "add-competitor" | "remove-competitor" | "set-competitor-price", competitorPrice?: string) => {
    ...
    try {
      const response = await fetch("/api/target-prices", {
        ...
        body: JSON.stringify({ action, sku: row.sku, nmId: row.nmId, competitorNmId, competitorPrice }),
      });
      const data = await response.json() as TargetPricesResponse;
            ...
            <section className="pricing-panel">
              ...
              {selectedPricingRow && (() => {
                ...
                return <div className="pricing-modal-backdrop" role="presentation" onMouseDown={() => setSelectedPricingRow(null)}><section className="pricing-modal" role="dialog" aria-modal="true" aria-label={`Конкуренты ${selectedPricingRow.sku}`} onMouseDown={(event) => event.stopPropagation()}>
                  ...
                  <div className="pricing-competitor-list"><h4>В сравнении</h4>{selectedPricingRow.competitors.map((competitor) => <article key={competitor.nmId}><div><strong>{competitor.name || `Карточка ${marketplaceCode} ${competitor.nmId}`}</strong><small>{competitor.source || "добавлен в мониторинг"}{competitor.updatedAt ? ` · ${formatDateTime(competitor.updatedAt)}` : ""}</small>{competitor.error && <em>{competitor.error}</em>}</div><div className="pricing-competitor-actions">{isOzon && canManage && competitor.nmId > 0 && <label className="pricing-competitor-price"><span>Цена, ₽</span><input value={competitorPriceDrafts[competitor.nmId] ?? ""} onChange={(event) => setCompetitorPriceDrafts((current) => ({ ...current, [competitor.nmId]: event.target.value.replace(/[^0-9,.]/g, "") }))} inputMode="decimal" placeholder="0" /></label>}{!isYandex && competitor.nmId > 0 && <a href={isOzon ? `https://www.ozon.ru/product/${competitor.nmId}` : `https://www.wildberries.ru/catalog/${competitor.nmId}/detail.aspx`} target="_blank" rel="noreferrer">{competitor.price ? formatMoney.format(competitor.price) : "Нет цены"} ↗</a>}{(isYandex || competitor.nmId < 0) && <span>{competitor.price ? formatMoney.format(competitor.price) : "Нет цены"}</span>}{isOzon && canManage && competitor.nmId > 0 && <button type="button" className="pricing-competitor-save" onClick={() => void updatePricingCompetitor(selectedRow, competitor.nmId, "set-competitor-price", competitorPriceDrafts[competitor.nmId])} disabled={!competitorPriceDrafts[competitor.nmId] || pricingCandidateUpdatingId === competitor.nmId}>{pricingCandidateUpdatingId === competitor.nmId ? "…" : "Сохранить"}</button>}{canManage && competitor.nmId > 0 && <button type="button" className="pricing-competitor-remove" onClick={() => void updatePricingCompetitor(selectedRow, competitor.nmId, "remove-competitor")} disabled={pricingCandidateUpdatingId === competitor.nmId}>{pricingCandidateUpdatingId === competitor.nmId ? "…" : "Убрать"}</button>}</div></article>)}{!selectedPricingRow.competitors.length && <p>Для этой карточки пока не назначены конкуренты.</p>}</div>
                  {canManage && <section className="pricing-candidate-picker"><div className="pricing-candidate-heading"><div><span className="section-kicker">КОНКУРЕНТЫ</span><h4>Добавить вручную</h4><p>{isOzon ? "Минимальную цену аналогов Ozon подтягиваем автоматически по ценовому индексу — стратегия не нужна. Можно также добавить ID конкретной карточки и указать её цену вручную: она сразу пойдёт в расчёт таргета." : isYandex ? "Вставь числовой ID карточки Яндекс Маркета. Цены появятся после обновления цен." : "Вставь артикул WB нужной карточки. Цена конкурента появится после отдельного обновления цен."}</p></div></div>
