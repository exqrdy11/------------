...
export default function Home() {
  ...
  const fulfillmentRows = useMemo(() => {
    ...
    const selectedQuantity = (row: StockRow) => {
      if (fulfillmentList === "physical") return row.ffStock[warehouseId] ?? 0;
            ...
            <section className="fulfillment-panel">
              {!selectedFulfillmentWarehouse ? <>
                ...
                <div className="fulfillment-metric-grid fulfillment-metric-grid-five" role="group" aria-label="Списки по статусу товара">
                  <button type="button" className={`fulfillment-metric physical ${fulfillmentList === "physical" ? "active" : ""}`} aria-pressed={fulfillmentList === "physical"} onClick={() => { setFulfillmentList("physical"); setQuery(""); }}><span>Фактически на ФФ</span><strong>{formatNumber.format(selectedFulfillmentWarehouse.physicalStock)} <small>шт.</small></strong><p>Включая новые FBS: они ещё лежат на ФФ</p></button>
