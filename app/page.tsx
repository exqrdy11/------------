...
function activeFbsQuantity(row: StockRow) {
  return row.fbs + row.receiving + row.toSale;
}
...
export default function Home() {
  ...
  const activeFbsByLocation = useMemo(() => rows.reduce<FbsBreakdown>((total, row) => {
    for (const source of [row.fbsByLocation, row.receivingByLocation, row.toSaleByLocation]) {
  ...
  const fulfillmentListMeta: Record<FulfillmentList, { kicker: string; title: string; empty: string; primary: string; footer: string }> = {
    available: { kicker: "ОСТАТКИ НА ФФ", title: "Доступный остаток по артикулам", empty: "На этом ФФ нет доступного остатка", primary: "Доступно ФФ", footer: "Из остатка вычтены только заказы со статусом «на сборке»" },
    reserved: { kicker: "НА СБОРКЕ FBS", title: "Заказы FBS на сборке", empty: "Нет заказов FBS на сборке", primary: "На сборке FBS", footer: "Статус confirm: эти единицы ещё на ФФ и вычтены из доступного остатка" },
    receiving: { kicker: "ПЕРЕДАНЫ WB", title: "Товары в доставке до WB", empty: "Нет товаров в доставке до WB", primary: "Переданы WB", footer: "Статус complete + waiting: это уже отправленный заказ, он не вычитается второй раз" },
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesQuery = !term || row.name.toLowerCase().includes(term) || row.sku.toLowerCase().includes(term) || String(row.nmId ?? "").includes(term);
      const matchesFilter = filter === "Все" || (filter === "Дефицит" && row.status !== "В норме") || (filter === "Активные FBS" && hasFbsMovement(row));
  ...
  const counts = useMemo(() => ({
    ...
    transit: rows.filter(hasFbsMovement).length,
  }), [rows]);
          ...
          {activeView === "cabinets" ? (
            ...
            <section className="fulfillment-panel">
              {!selectedFulfillmentWarehouse ? <>
                ...
                <div className="fulfillment-export-note"><span>Только актуальные FBS-заказы этого ФФ. Одна строка — один заказ: артикул, количество и стикер WB; включены статусы «на сборке» и «в доставке».</span>{ffOrdersExportMessage && <strong className="success">{ffOrdersExportMessage}</strong>}{ffOrdersExportError && <strong className="error">{ffOrdersExportError}</strong>}</div>
                <div className="fulfillment-metric-grid" role="group" aria-label="Списки по статусу товара">
                  <button type="button" className={`fulfillment-metric ${fulfillmentList === "available" ? "active" : ""}`} aria-pressed={fulfillmentList === "available"} onClick={() => { setFulfillmentList("available"); setQuery(""); }}><span>Доступно на ФФ</span><strong>{formatNumber.format(selectedFulfillmentWarehouse.stock)} <small>шт.</small></strong><p>WB FBS {formatNumber.format(selectedFulfillmentWarehouse.physicalStock)} · на сборке {selectedFulfillmentWarehouse.fbs}</p></button>
                  <button type="button" className={`fulfillment-metric ${fulfillmentList === "reserved" ? "active" : ""}`} aria-pressed={fulfillmentList === "reserved"} onClick={() => { setFulfillmentList("reserved"); setQuery(""); }}><span>На сборке FBS</span><strong>{formatNumber.format(selectedFulfillmentWarehouse.fbs)} <small>шт.</small></strong><p>confirm · вычтено из доступного</p></button>
                  <button type="button" className={`fulfillment-metric ${fulfillmentList === "receiving" ? "active" : ""}`} aria-pressed={fulfillmentList === "receiving"} onClick={() => { setFulfillmentList("receiving"); setQuery(""); }}><span>Переданы WB</span><strong>{formatNumber.format(selectedFulfillmentWarehouse.receiving)} <small>шт.</small></strong><p>complete + waiting · без второго вычета</p></button>
