...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ warehouse: await updateFfWarehouse({ cabinetId: cabinet, id: payload.id, city: payload.city, name: payload.name, position: Number(payload.position) || 0 }) });
  } catch {
