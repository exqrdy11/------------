...
export async function POST(request: Request) {
  ...
  if (body?.action === "add-competitor" || body?.action === "remove-competitor" || body?.action === "set-competitor-price" || body?.action === "refresh-competitor") {
    if (session.role !== "owner" && !(session.role === "yandex-manager" && session.cabinetId === "yandex") && !(session.role === "wb-manager" && session.cabinetId === "metanutrix")) return NextResponse.json({ error: "Нет доступа к изменению конкурентов этого кабинета" }, { status: 403, headers: { "Cache-Control": "no-store" } });
